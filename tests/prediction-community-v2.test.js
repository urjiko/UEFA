...
const routeShell = fs.readFileSync(path.join(root, 'prediction-route-shell.js'), 'utf8');
...
assert.match(resultsUi, /Başka Bir Takımı Tahmin Et/);
assert.match(resultsUi, /community-other-team-button/);
assert.match(resultsUi, /window\.location\.replace/);
...
assert.match(resultsUi, /ensureVerticalChart/);
assert.match(resultsUi, /community-outcome-chart/);
assert.match(resultsUi, /community-outcome-rail/);
assert.match(resultsUi, /fill\.style\.height/);
assert.match(resultsUi, /community-average-bars-source/);
assert.match(resultsUi, /venueForCard/);
assert.match(resultsUi, /community-team-crest/);
assert.match(resultsUi, /text === 'İç saha' \|\| text === 'Deplasman'/);
assert.match(resultsUi, /leadMargin/);
assert.match(resultsUi, /community-card-crest-watermark/);
assert.match(resultsUi, /ucldraw:community-average-rendered/);
assert.doesNotMatch(resultsUi, /new MutationObserver/);
assert.match(resultsUi, /description\.textContent !== desiredDescription/);
...
assert.match(resultsCss, /community-outcome-plot/);
assert.match(resultsCss, /community-outcome-rail/);
assert.match(resultsCss, /community-card-crest-watermark/);
assert.match(resultsCss, /community-average-bars-source\s*\{[^}]*display:\s*none\s*!important/s);
assert.match(resultsCss, /max-width:\s*58px/);
assert.match(resultsCss, /max-width:\s*28px/);
assert.match(resultsCss, /body\[data-league="uecl"\].*#58c4d6/s);
assert.match(resultsCss, /var\(--league-background/);
assert.match(resultsCss, /community-results-hero > div > span[\s\S]*text-transform:\s*none/);
assert.doesNotMatch(resultsCss, /background:\s*#07111b/);
assert.doesNotMatch(resultsCss, /#62d89b|#e67d88|#57d17a/i);
...
assert.match(config, /history\.replaceState\(\{[\s\S]*communityAverage:\s*true/);
assert.match(config, /updateHistory:\s*false/);
assert.match(config, /resetRouteToRoot/);
assert.match(config, /#changeTeamButton/);
assert.match(config, /ucldrawSelection:\s*true/);
assert.match(config, /\.then\(\(\) => announceAverageRendered\(\)\)/);
...
assert.match(routeShell, /announceAverageRendered/);
assert.match(routeShell, /ucldraw:community-average-rendered/);
...
console.log('Community results preserve league backgrounds, reliable filters and clean route history.');
